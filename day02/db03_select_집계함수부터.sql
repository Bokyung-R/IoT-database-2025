...
-- 3-17 : 고객이 주문한 도서의 총 판매액, 평균값, 최고가를 구하시오.
...
-- 그롭화 GROUP BY 키워드 사용
-- 3-19 고객별로 주문한 도서의 총수량과 총 판매앨 구하시오. 
-- GROUP BY 를 사용하면 반드시 집계함수 및 GROUP BY 에 포함된 칼럼이 SELECT 안에 들어가 있어야함.
-- GROUP BY에 있는 컬럼만 SELECT 안에 사용할 ㅅ수 있나
SELECT custid
...
-- 추가 : 3-19의 내용에서 고객별총액을 내림차순으로 촐력하시오
SELECT custid
	 , COUNT(*) AS 구매도서수량
     , SUM(saleprice) AS 고객별총액
  FROM Orders
...
-- 3-20 : 가격이 8,000원 이상인 도서를 구매한 고객에 대하여 고객별 주문도서의 총 수량을 구하시오.
...
-- 3-27 : 도서를 구매하지않은 고객을 포함하여 고객의 이름과 고객이 주문한 도서의 판매가격을 구하시오
-- LEFT OUTER JOIN // RIGHT OUTER JOIN - LEFT와 RIGHT는 기준이 되는 테이블의 위치
SELECT *
  FROM Customer AS c
...
-- EXISTS : 상관서브쿼이에서 사용하는 키워드. 상관서브쿼리가 아니라도 사용가능
...
SELECT *
 ...
 WHERE EXISTS (
				SELECT *				-- o.custid 로 사용하지 않아도 됨
				  FROM Orders AS o
				 WHERE o.custid = c.custid -- 메인 쿼리의 컬럼이 서브쿼리에 사용되고 있음 : 상관 부속 질의
				 );
