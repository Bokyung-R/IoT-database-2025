...
-- 파이썬, C# 등프로그래밍 언어로 가져갈땐, 칼럼이름 컬럼갯수를 모두 파악해야하기때문에 아래와 같이 사용
SELECT bookid, bookname, publisher, price
  FROM Book;
  
-- 3-3 : 도서 테이블의 모든 출판사를 검색하시오.
-- 출반사별로 한번만 출력하면 좋겠음
-- ALL : 전부 다 // DICTINCT : 중복제거
SELECT DISTINCT publisher
  FROM Book;
  
-- 3-4 : 도서 중 가격이 20,000원 미만인 것을 검색하시오.
-- > 미만 | < 초과 | >= 이하 | <= 이상 | <> 같지않다 | != 같지않다 | = 같다 (프로그래밍언어와 차이)
SELECT *
  FROM Book
 WHERE price < 20000;
 
 -- 3-5 : 가격이 10,000원이상 20,000원이하인 도서를 검색하시오.
 SELECT *
   FROM Book
 ...
 SELECT publisher
  ...
  -- [^] : 1개의 문자가 일치하지 않는 것
  -- _ : 특정위치의 1개의 문자가 일치할 때 // _구% : 첫번째글자는 뭐든 상관없고, 두번째 글짜는 구, 그 뒤로는 상관없음
   SELECT *
   FROM Book
  WHERE bookname LIKE '[0-11]';

-- 추가 : 고객중에서 전화번호는 사람을 검색하시오.
SELECT *
  FROM Customer
