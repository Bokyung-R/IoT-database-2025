...
-- 테이블 생성
-- 3-34 : NewBook 테이블 생성하세요. 정수형은 Integer 사용, 문자형을 Varchar를 사용하십시오.
-- 기본키를 설정합니다.
-- 기본키가 2개 이상일 경우 아래와 같이 작성해야함.
CREATE TABLE NewBook (
	boodId 		INTEGER ,
    boodName	VARCHAR(255),
...
-- 기본키가 2개 이상일 경우 컬럼에 PRIMARY KEY 두군데 이상 작성 불가
CREATE TABLE NewBook (
	boodId 		INTEGER	PRIMARY KEY,
    boodName	VARCHAR(255),
    publisher 	VARCHAR(50),
    price		INTEGER
);
...
-- 테이블 생성시, 제약조건을 추가가능
-- booname은 NULL을 가질 수 없고, publisher는 같은 값이 있으면 안됨.
...
-- 3-35 : 아래 속성의 NewCustomr 테이블을 생성하시오.
-- custid - INTEGER, 기본키
