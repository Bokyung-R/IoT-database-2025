...
from tkinter import *   # tkinter 는 이걸로 모든 모둘을 부를 수 없음
...
def showDatas():
    '''
    데이터 베이스 내 모든 학생 정보를 가져와 표시하는 함수\n
    매개변수 필요없음
    '''
    global dataView     # 외부에 있는 변수를 전역변수로 사용 선언

    ### DB 연결. 커넥션 객체생성 -> 커서 -> 쿼리 실행 -> 커서로 데이터 패치 -> 커서종료 -> 커넥션 종료
...
# 6. getData(event) - 트리뷰 한줄 더블 클릭한 값 엔드티에 표시
def getData(event):
    '''
    트리뷰 더블 클릭으로 선택된 학생정보를 Entry 위젯에 채우는 사용자 함수
    
    Args:
        event : 트리부에 발생하는 이벤트 객체
    '''

    global endt1, ent2, ent3, ent4, dataView    # 전역 변수로 사용 선언

    ## 엔트리 위젯 기존 내용 삭제
    ent1.config(state='normal') # 데이터 들어갈 수 있게 해주고
    ent1.delete(0,END) # 학생번호 기존 데이터 삭제
    ent1.config(state='readonly')   # 다시 readonly로
    ent2.delete(0,END) # 학생명 기존 데이터 삭제
    ent3.delete(0,END) # 전화번호 기존 데이터 삭제
    ent4.delete(0,END) # 입학년도 기존 데이터 삭제

    ## 트리뷰 선택 항목 ID 가져오기('I001')
    ...
    ent3.insert(0, item_values[2])  # 전화번호
...
def addData():
    ...
    try:
        ...
        conn.commit()   # 트랜잭션 확정
        lastid = cursor.lastrowid   # 마지막에 isnert 된 레코드의 id를 가져옴(auto_increment)
        print(lastid)
...
myFont = Font(family='NanumGotic', size=10)     # 이후에 화면 위젯에 지정할 동일 폰트 생성

# 4. UI 구성
## 4-1. 레이블
tk.Label(root, text='학생번호', font=myFont).place(x=10,y=10)
tk.Label(root, text='학생명', font=myFont).place(x=10,y=40)
tk.Label(root, text='전화번호', font=myFont).place(x=10,y=70)
...
ent3.place(x=140, y=70) # 전화번호 Entry
...
# 6. 트리뷰 항목 더블 클릭하면 이벤트 발생(getData()함수 호출)
