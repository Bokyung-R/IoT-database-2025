...
select concat(e1.first_name,' ',e1.last_name) as 'name',
 ...
 where (e1.job_id, e1.salary) in (select e1.job_id,
...
/* 샘플문제 1 - 프로젝트 성공으로 급여 인상이 결정됨.
				사원 현재 107명 19개 업무에 소속되어 근무중. 회사 업무 Distince_job 5개 업무에서 일하는 사원
...
/* 샘플 - 부서와 업무별 급여합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
		  부서 번호와 업무를 기준으로 정해 행을 그룹별로 나우어 급여 합계와 인원수를 출력(20행)
*/
...
-- RANK
/* 샘플 - 분석함수 NTLE() 사용, 부서별 급여 합계를 구하시오, 급여가 제일 큰 것이 1, 제일 작은것이 4가 되도록 등급을 나눔(12행) */
...
select employee_id
     ...
     , salary
     , department_id
     , rank() over (partition by department_id order by salary desc) as 'Rank' -- ex) 공동 1등이면 그 다음은 3등으로 (2등없음) // 1 1 3 순위 매기기 rank
     , dense_rank() over (partition by department_id order by salary desc) as 'Dense Rank' -- ex) 공동 1등이면 그 다음은 2등으로  // 1 1 2 순위 매기기 dense_rank
     , row_number() over (partition by department_id order by salary desc) as 'Row_Number' -- 그냥 행 번호 매기기
  from employees
